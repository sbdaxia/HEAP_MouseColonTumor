---
title: "CDF analysis_HEAP-09282019 with merged peaks"
author: "Bing Shui"
date: "`r Sys.Date()`"
output: 
  html_document:
...
```{r}
peaks.mir <- readRDS("Datafiles/merged-peaks-mirs-200-09282019.rds")
```
...
hva_id <- read_csv("no.UniprotID.list_2_Uniprot_HVA_filtered.csv")
hvak_id <- read_csv("no.UniprotID.list_2_Uniprot_HVAK_filtered.csv", col_names = FALSE)

no_id <- no_id %>% left_join(hva_id, by = c("gene_name" = "gene_names")) %>% left_join(hvak_id, by = c("gene_name" = "X1"))
...
# save the datafile
saveRDS(peaks.mir, "Datafiles/merged-peaks-mirs-200-09282019-withID.rds")
```
...
saveRDS(mirs.peaks, "Datafiles/miRNA-merged-peaks-list-09282019-withIDs.rds")
```
...
include_graphics('merged_RNA_CDF.png')

pdf('PDF_figure/CDF_analysis_merged/merged_RNA_CDF.pdf',
    height = 8,
    width = 14)
plotCDF.ggplot(rna_DGE,
               list(rna_DGE$gene, 
...
include_graphics('merged_Protein_CDF.png')

pdf('PDF_figure/CDF_analysis_merged/merged_Protein_CDF.pdf',
    height = 8,
    width = 14)
plotCDF.ggplot(protein_DGE,
               list(protein_DGE$gene, 
...
include_graphics('merged_CLIP_CDF.png')

pdf('merged_CLIP_CDF.pdf',
    height = 8,
    width = 14)
plotCDF.ggplot(clip_DGE,
               list(clip_DGE$gene, 
...
p <- ggplot(mirna.family.DGE, aes(x = log2(baseMean), y = peak.number, label = miR.family, size = peak.number)) +
